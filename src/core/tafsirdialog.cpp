...
void TafsirDialog::setShownVerse(const Verse &newShownVerse)
{
    m_shownVerse = newShownVerse;
}

void TafsirDialog::btnNextClicked()
{
    ...
    m_shownVerse.page = m_dbMgr->getVersePage(m_shownVerse.surah, m_shownVerse.number);
    loadVerseTafsir();
}

void TafsirDialog::btnPrevClicked()
{
...
void TafsirDialog::setupConnections()
{
    connect(ui->btnNext,
            &QPushButton::clicked,
            this,
...
void TafsirDialog::setTafsirAsTitle()
{
    QString title;
    DBManager::Tafsir id = qvariant_cast<DBManager::Tafsir>(m_settings->value("Reader/Tafsir"));
    switch (id) {
    case DBManager::adwa:
...
void TafsirDialog::loadVerseTafsir()
{
    QString title = tr("Surah: ") + m_dbMgr->getSurahName(m_shownVerse.surah) + " - "
                    + tr("Verse: ") + QString::number(m_shownVerse.number);
    QString glyphs = m_dbMgr->getVerseGlyphs(m_shownVerse.surah, m_shownVerse.number);
    QString fontFamily = m_fontPrefix + QString::number(m_shownVerse.page).rightJustified(3, '0');
    ...
    QFont sideFont = qvariant_cast<QFont>(m_settings->value("Reader/SideContentFont"));
    ui->textEdit->setFont(sideFont);
    ui->textEdit->setHtml(m_dbMgr->getTafsir(m_shownVerse.surah, m_shownVerse.number));
...
void TafsirDialog::closeEvent(QCloseEvent *event)
{
    this->hide();
}

void TafsirDialog::showEvent(QShowEvent *event)
{
    setTafsirAsTitle();
