...
void SearchDialog::getResults()
{
    ...
    if (!ui->chkSurahsOnly->isChecked()) {
        ...
        m_currResults = m_dbMgr->searchVerses(m_searchText, range, ui->chkWholeWord->isChecked());
    } else {
        m_currResults = m_dbMgr->searchSurahs(m_searchText,
...
void SearchDialog::setupConnections()
{
    connect(ui->btnSrch,
            ...
            Qt::UniqueConnection);
    connect(ui->btnNext, &QPushButton::clicked, this, &SearchDialog::moveFwd, Qt::UniqueConnection);
    connect(ui->btnPrev, &QPushButton::clicked, this, &SearchDialog::moveBwd, Qt::UniqueConnection);
