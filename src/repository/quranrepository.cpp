...
bool
QuranRepository::executeQuery(QSqlQuery& query) const
{
  bool success = query.exec();
  if (!success)
    qCritical("Failed to execute SQL statment, query: %s, error: %s",
...
QuranRepository::open()
{
  ...
  if (!QSqlDatabase::open())
    qFatal("Failed to open quran DB, file: %s", qPrintable(m_assetsDir.absoluteFilePath("quran.db")));
}
...
QuranRepository::getRubStartingInPage(const int page) const
{
  ...
  QSqlQuery dbQuery(*this);
  dbQuery.prepare("SELECT rub % 4, hizb FROM verses_v1 GROUP BY rub HAVING MIN(page) = ?");
  dbQuery.addBindValue(page);

  executeQuery(dbQuery);
  if (dbQuery.next())
    result.emplace(std::make_pair(dbQuery.value(0).toInt(), dbQuery.value(1).toInt()));
...
QuranRepository::getVersePage(const int& surahIdx, const int& verse) const
{
  ...
  dbQuery.prepare(query.arg(QString::number(m_config.qcfVersion()), QString::number(surahIdx), QString::number(verse)));
...
QuranRepository::getJuzStart(const int juz) const
{
  ...
  QString query =
    QString::asprintf("SELECT page,sura_no,aya_no FROM verses_v%i WHERE jozz=%i", m_config.qcfVersion(), juz);
  dbQuery.prepare(query);

  executeQuery(dbQuery);
  dbQuery.next();

  return Verse(dbQuery.value(0).toInt(), dbQuery.value(1).toInt(), dbQuery.value(2).toInt());
}
...
QuranRepository::getVerseJuz(const Verse verse) const
{
  ...
  QString query = "SELECT jozz FROM verses_v1 WHERE page=? AND sura_no=? AND aya_no=?";
  dbQuery.prepare(query);
...
QuranRepository::verseInfoList(const int page) const
{
  ...
  dbQuery.prepare(query.arg(QString::number(m_config.qcfVersion()), QString::number(page)));

  executeQuery(dbQuery);

  while (dbQuery.next()) {
    viList.append(Verse(page, dbQuery.value(0).toInt(), dbQuery.value(1).toInt()));
  }
...
{
  QSqlQuery dbQuery(*this);
  dbQuery.prepare("SELECT sura_no,aya_no FROM verses_v" + QString::number(m_config.qcfVersion()) +
...
QuranRepository::surahStartPage(int surahIdx) const
{
  ...
  dbQuery.prepare("SELECT page FROM verses_v" + QString::number(m_config.qcfVersion()) +
...
{
  QSqlQuery dbQuery(*this);
  dbQuery.prepare("SELECT page,sura_no,aya_no FROM verses_v" + QString::number(m_config.qcfVersion()) + " WHERE id=:i");
...
QuranRepository::searchSurahNames(QString text) const
{
  QList<int> results;
  QSqlQuery dbQuery(*this);
  QString q = "SELECT DISTINCT sura_no FROM verses_v1 WHERE (sura_name_ar like '%" + text +
              "%' OR "
              "sura_name_en like '%" +
              text + "%')";
...
QList<Verse>
QuranRepository::searchSurahs(QString searchText, const QList<int> surahs, const bool whole) const
{
  QList<Verse> results;
  QSqlQuery dbQuery(*this);

  QString q = "SELECT page,sura_no,aya_no FROM verses_v" + QString::number(m_config.qcfVersion()) + " WHERE (";
  ...
  if (whole)
    q.append(") AND (aya_text_emlaey like '" + searchText + " %' OR aya_text_emlaey like '% " + searchText +
             " %') ORDER BY id");
  else
...
QList<Verse>
QuranRepository::searchVerses(QString searchText, const int range[], const bool whole) const
{
  QList<Verse> results;
  QSqlQuery dbQuery(*this);

  QString q = "SELECT page,sura_no,aya_no FROM verses_v" + QString::number(m_config.qcfVersion()) +
              " WHERE (page >= " + QString::number(range[0]) + " AND page <= " + QString::number(range[1]) + ")";
...
QuranRepository::randomVerse() const
{
  ...
  dbQuery.prepare("SELECT page,sura_no,aya_no FROM verses_v" + QString::number(m_config.qcfVersion()) +
...
QuranRepository::next(const Verse& verse, bool withBasmallah) const
{
  ...
  if (withBasmallah && nxt.number() == 1 && nxt.surah() != 9 && nxt.surah() != 1)
    nxt.setNumber(0);
