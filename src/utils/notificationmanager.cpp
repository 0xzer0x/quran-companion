...
void NotificationManager::checkDailyVerse()
{
    ...
    if (!timestamp.exists()) {
        ...
        if (lastTimestamp.daysTo(m_dtNow) > 0) {
            ...
            m_votd.first = Verse{data.at(0).toInt(), data.at(1).toInt(), data.at(2).toInt()};
            m_votd.second = m_dbMgr->getVerseText(m_votd.first.surah, m_votd.first.number);
            setVotdMsg();
        }
        timestamp.close();
    }
...
QString NotificationManager::votdStringEntry() const
{
    QString entry = QString::number(m_votd.first.page).rightJustified(3, '0') + ":"
...
void NotificationManager::addActions()
{
    ...
    connect(prefs, &QAction::triggered, this, &NotificationManager::openPrefs, Qt::UniqueConnection);
    connect(exit, &QAction::triggered, this, &NotificationManager::exit, Qt::UniqueConnection);
    connect(about, &QAction::triggered, this, &NotificationManager::openAbout, Qt::UniqueConnection);
    connect(update,
            &QAction::triggered,
            this,
...
void NotificationManager::setVotdMsg()
{
    ...
    msg.append(tr("Surah: ") + m_dbMgr->getSurahName(m_votd.first.surah) + " - " + tr("Verse: ")
...
QPair<Verse, QString> NotificationManager::votd()
{
    if (!m_votdShown)
        checkDailyVerse();
