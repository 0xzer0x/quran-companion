...
TafsirTask::url() const
{
  return QUrl::fromEncoded(
    ("https://github.com/0xzer0x/quran-companion-extras/raw/main/tafasir/" + m_tafasir.at(m_idx).filename())
      .toLatin1());
}
...
{
  return QFileInfo(m_downloadsDir.absoluteFilePath("tafasir/" + m_tafasir.at(m_idx).filename()));
}
